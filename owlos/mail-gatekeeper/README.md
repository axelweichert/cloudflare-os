...
```bash
npx tsx --test owlos/mail-gatekeeper/approval-queue.test.ts \
                owlos/mail-gatekeeper/mcp-server.test.ts
...
```bash
npx wrangler dev -c owlos/mail-gatekeeper/wrangler.jsonc --port 8799 --local
...
```bash
npx wrangler deploy -c owlos/mail-gatekeeper/wrangler.jsonc
```
