...
#   (1) STRUKTUR   — unser owlos/-Verzeichnis + Kern-Dokumente/Skripte.
...
#                    (unsere direkte EN→DE-Ersetzung, siehe owlos/I18N-DE.md).
#
# Verwendung:
#   owlos/scripts/fork-guard.sh            # normaler Lauf (CI / lokal)
#   owlos/scripts/fork-guard.sh --verbose  # zeigt jeden geprüften Marker
#
# Rein lesend. Kein git-Zustand wird verändert. Kein Netzwerkzugriff.
# Prozess & Register: siehe owlos/FORK-SYNC.md
#
set -euo pipefail
...
STRUCT_MARKERS=(
  "owlos"                                 # das Fork-Wurzelverzeichnis überhaupt
  "owlos/FORK-SYNC.md"                 # Sync-Runbook (dieser Prozess)
  "owlos/I18N-DE.md"                   # DE-Glossar / Übersetzungsstrategie
  "owlos/scripts/drift-report.sh"      # Drift-Report (read-only)
  "owlos/scripts/fork-guard.sh"        # dieser Wächter selbst
...
if [ "$FAILURES" -eq 0 ]; then
  ...
  echo "    git tag --list 'owlos/pre-sync-*'"
  echo "    git reset --hard owlos/pre-sync-<DATUM>   # nur auf dem Sync-Branch!"
  echo "  Details: owlos/FORK-SYNC.md, Abschnitt 'Null-Verlust-Garantie'."
  echo "==================================================================="
  exit 1
fi
