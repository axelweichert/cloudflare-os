// vonBuschOS — CRM-Gatekeeper: CRM-Datenzugriff (VON-1800 / K2)
//
// Kapselt Lese- und Schreibzugriff auf das CRM (vonbusch-crm-eu D1). Reads sind direkt
// erlaubt (read-only, gefahrlos); Writes laufen ausschließlich über bereits freigegebene
// `WriteAction`s aus der Approval-Queue (siehe write-queue.ts / worker.ts).
//
...
//
// Sicherheit: Spaltennamen stammen NUR aus der Allowlist (write-queue.ts), Werte werden
// ausschließlich als gebundene Parameter (?) übergeben — kein String-Interpolieren von
// Nutzdaten in SQL. Tabellennamen sind statisch pro Entity gemappt.

import type { CrmEntity, WriteAction, CrmValue } from "./write-queue.ts";
