...
```bash
npx tsx --test owlos/crm-gatekeeper/write-queue.test.ts \
                owlos/crm-gatekeeper/crm-store.test.ts \
                owlos/crm-gatekeeper/mcp-server.test.ts
...
sed 's/REPLACE_WITH_VONBUSCH_CRM_EU_ID/00000000-0000-0000-0000-000000000000/' \
  owlos/crm-gatekeeper/wrangler.jsonc > /tmp/crm.e2e.jsonc
npx wrangler d1 execute CRM_DB --local -c /tmp/crm.e2e.jsonc --file owlos/crm-gatekeeper/schema.sql
...
```bash
npx wrangler deploy -c owlos/crm-gatekeeper/wrangler.jsonc
```
