...
| `38892c0` #341 | git-storage crash recovery, loose ends | **a** | **nein** (real: konfliktfrei auto-merged; DE-Fläche minimal) | ✅ übernommen `4aa450d` (Sub-Task `c3ad3074`, PR gegen `main`, 2026-08-28) | Datenintegrität/Crash-Recovery (transaktionale Persistenz von Chat-Log + Änderungen). **Heuristik überschätzte die Kollision**: trotz 1248/-631 Zeilen Refactor divergiert unser Fork auf diesen Dateien nur um ~19 reine DE-Strings (0 Logik) → 3-Wege-Merge sauber, alle DE-Strings erhalten, keine EN-Regression. |
...
- **Übernommen (a):** `38892c0` #341 *Clean up loose ends in git-storage, improving crash recovery* → `4aa450d` (Sub-Task `c3ad3074`, Branch `sync/341-git-storage-crash-recovery`, PR gegen `main`).
  - **Kollision real ≠ Heuristik:** Betreff-Heuristik markierte agent.ts/overseer.ts als Konflikt-Risiko. Messung vor dem Pick (`git diff 38892c0^ origin/main`) zeigte: unser Fork divergiert auf diesen Dateien nur um **5 (agent.ts) + 14 (overseer.ts) reine DE-String-Zeilen, 0 Logik**; api.ts nur um unsere DeepSeek-V4-Pro-Entfernung. → `git cherry-pick -x` **konfliktfrei auto-merged** (3-Wege: Base=`38892c0^`, ours=DE-Strings, theirs=Upstream-Logik überschneidungsfrei).
  - **Null-Verlust verifiziert:** alle 19 DE-Strings post-merge vorhanden (grep), keine EN-Regression (`(title unavailable)`/`Web fetch:` etc. = 0 Treffer), api.ts-Delta vs. Upstream = ausschließlich unsere Modell-Entfernung.
  - Verifikation: `fork-guard.sh` grün (12 Marker, 25 DE-Dateien) · `workshop-backend` + `workshop-shared` + `workshop-frontend` `tsc --noEmit` = 0. Bundle-Test `chat-changes.test.ts` (workerd/vitest-pool-workers) läuft in der CTO-Sandbox nicht an (Pool startet nicht — **identischer Fehler auf pre-pick `main`**, also umgebungsbedingt, keine Pick-Regression); definitiver Grün-Nachweis via CI-Check „Build and test" auf dem PR.
- **Register-Hinweis:** Betreff-Heuristik hat #298 als „konfliktarm" fehlklassifiziert (real: 3 DE-Frontend-Konflikte) **und** #341 umgekehrt als „Konflikt" überschätzt (real: konfliktfrei). Lehre: Kollisions-Spalte nicht blind vertrauen — vor dem Pick `git diff <sha>^ origin/main -- <datei>` gegen die DE-Dateien messen (Logik- vs. String-Divergenz), nicht nur `git show --stat`.
