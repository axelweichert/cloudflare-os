// Minimal-UI für den "Kunde anlegen"-Workflow. Ein Formular (Firma + optionale Details), ein
// Knopf, und die Liste der zuletzt gestarteten Läufe. Der eigentliche Workflow läuft serverseitig
// im gespawnten Agenten — dieses UI ist nur der Auslöser für Nicht-Techniker.
...
<div class="wrap">
  <h1>Kunde anlegen</h1>
  <p class="sub">Firma eintragen — der Kunden-Agent prüft owlOS auf Dubletten und legt den Kunden approval-pflichtig an.</p>
  <label for="firma">Firma</label>
  <input id="firma" placeholder="z. B. Musterbau GmbH">
  <label for="details">Zusatzangaben (optional)</label>
  <textarea id="details" placeholder="z. B. Ansprechpartner, USt-IdNr., Adresse — nur wenn bekannt"></textarea>
  <button id="go">Kunde anlegen</button>
  ...
  try {
    ...
    $("runs").innerHTML = runs.length
      ? "<label>Zuletzt gestartet</label>" + runs.map((r) =>
          `<div class="run"><b>${esc(r.title)}</b><small>${esc(r.details || "—")} · ${new Date(r.at).toLocaleString("de-DE")}</small></div>`).join("")
      : "";
