# Kunde anlegen — owlOS-Workflow-Blueprint

Ein teilbarer Blueprint statt eines Copy-&-Paste-Einzelprompts. Wer ihn instanziiert, bekommt ein
kleines Gadget mit einem Formular (Firma + optionale Details) und einem Knopf. Ein Klick spawnt
einen KI-Agenten mit dem festen Kunden-Anlage-Workflow gegen die verbundene owlOS-Instanz.

## Bindings

| Binding    | Typ                        | Zweck |
| ---------- | -------------------------- | ----- |
| `WORKFLOW` | agentSpawner               | spawnt den Kunden-Agenten; beim Instanziieren wählt der Nutzer nur das Modell |
...
- `GET  /api/companies` — Kundenliste lesen (direkt)
...
1. Nutzer trägt Firma (+ optionale Details) ein und klickt „Kunde anlegen".
2. `Gadget.launch()` ruft `env.WORKFLOW.spawn(titel, prompt)` mit dem kuratierten Workflow-Prompt.
3. Der Agent liest die Kundenliste (Dubletten-Check), legt den Kunden approval-pflichtig an und
   vergibt eine Kundennummer.
