...
// (`demo.owl-os.cloud`, OWL-1641): `/api/health` is public and returns `{app:"owlos-cloud-erp"}`;
// every other `/api/*` returns `401 {"error":"unauthorized"}` without a token.
//
// AUTH SCHEME: the demo API sits behind Cloudflare Access for its browser UI, so a demo token could
// not be minted from a headless build to read back the exact header name. Rather than guess Bearer
// vs X-API-Key (RATEN VERBOTEN), the connect flow probes BOTH against the live instance and persists
// whichever one the instance answers 200 to — `verifyCredentials()` never returns a scheme it did
// not see accepted, so nothing is guessed. S2 (OWL-1634) hard-codes the confirmed scheme once a token
// is available.
 ...
 *   2. GET {base}/api/me must go 401→200 under Bearer or X-API-Key; the accepted scheme is returned.
...
export async function verifyCredentials(
  ...
  for (const scheme of ["bearer", "apikey"] as const) {
    ...
    try {
      me = await fetchJson(`${base}/api/me`, { method: "GET", headers: authHeader(scheme, apiToken) });
    } catch (e: any) {
      throw new OwlosError(`Unable to reach ${base}: ${e?.message ?? e}`);
    }
...
export class OwlosClient {
  ...
  /** GET {base}/api/me — the authenticated workspace identity. Shape is passthrough (S2 types it). */
  async me(): Promise<Record<string, unknown>> {
    const { status, body } = await fetchJson(`${this.#creds.instanceUrl}/api/me`, {
      method: "GET",
    ...
    if (status !== 200) throw new OwlosError(`owlOS /api/me returned HTTP ${status}.`);
