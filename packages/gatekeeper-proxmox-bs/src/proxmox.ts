...
class ProxmoxSessionImpl extends RpcTarget implements ProxmoxVeSession {
  ...
  async version(): Promise<ProxmoxBsVersion> {
    return await this.#read<ProxmoxBsVersion>(PRODUCT.verifyPath, {
  ...
  async datastoreUsage(): Promise<ProxmoxBsDatastoreUsage[]> {
    const usage = await this.#read<ProxmoxBsDatastoreUsage[]>("/status/datastore-usage", {
      title: "Read Proxmox Backup Server datastore usage",
      description: "Read per-datastore usage (`GET /api2/json/status/datastore-usage`).",
    });
    return usage ?? [];
  }
}
