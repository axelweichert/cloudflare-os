...
export default function Connections({ overseer, gadget, chatId, authenticatedApi, onConnectionsChange, isVisible, onHasGatekeepersChange }: ConnectionsProps) {
  ...
  const loadGatekeepers = async () => {
    try {
      ...
      toasts.add({ title: 'Verbindungen konnten nicht geladen werden', variant: 'error' })
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleDeleteHookConfirm = async () => {
    ...
    try {
      ...
      toasts.add({ title: 'Hook konnte nicht gelöscht werden', variant: 'error' })
    } finally {
    ...
    return [
      {
        ...
        targetTitle: `${gadgetInfo.title} (dieses Gadget)`,
        name: 'GADGET',
      },
  ...
  const handleEditSave = async (name: string) => {
    ...
    if (!newName) {
      toasts.add({ title: 'Der Verbindungsname darf nicht leer sein', variant: 'error' })
      return
    }
    if (newName === name) {
    ...
    try {
      ...
      toasts.add({ title: 'Verbindungsname konnte nicht aktualisiert werden', variant: 'error' })
    } finally {
      setEditingBinding(null)
    }
  }
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      toasts.add({ title: 'Verbindung konnte nicht entfernt werden', variant: 'error' })
    } finally {
      ...
      <div className="mx-auto flex min-h-full w-full max-w-5xl flex-col px-4 py-5 sm:px-6">
        <section>
          <div className="mb-3 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            <div className="min-w-0">
              ...
              <p className="mt-1 text-[13px] leading-[18px] font-normal tracking-[-0.25px] text-kumo-subtle">
                Externe Ressourcen, die dieses Gadget nutzen kann.
              </p>
            </div>
            <WorkshopButton
              tone="primary"
          ...
          {loading ? (
            ...
            <EmptyState
              title="Keine verbundenen Ressourcen"
              description="Verbinde Google Docs, GitHub, Google Sheets und andere Dienste, damit dieses Gadget externe Daten sicher nutzen kann."
              actionLabel="Ressource verbinden"
                      ...
                      <div className="flex flex-wrap items-center gap-3">
                        <div className="min-w-0 flex-1">
                          <p className="truncate text-[13px] leading-[18px] font-medium tracking-[-0.25px] text-kumo-danger">
                            {gk.resourceTitle} löschen?
                          </p>
                          <p className="truncate text-[12px] leading-4 font-normal tracking-[-0.2px] text-kumo-subtle">
                            Die Verbindung <span className="font-mono">{gk.name}</span> wird aus diesem Gadget entfernt.
                          </p>
                        </div>
                        <WorkshopButton
                          tone="danger"
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div className="min-w-0 flex-1">
                          <p className="flex items-center gap-2 truncate text-[13px] leading-[18px] font-medium tracking-[-0.25px] text-kumo-default">
                            ...
                            {isPending && (
                              <Tooltip content="In diesem Chat hinzugefügt; bleibt erhalten, wenn du die Änderungen des Chats übernimmst" asChild>
                          ...
                          <p className="mt-0.5 truncate text-[11px] leading-4 tracking-[-0.1px] text-kumo-inactive">
                            Im Code referenziert als: <span className="font-mono text-kumo-subtle">{gk.name}</span>
                          </p>
                        </div>
                        <div className="ml-auto flex shrink-0 items-center gap-1">
                          <Tooltip content="Im Code verwendeten Namen bearbeiten" asChild>
                            <WorkshopIconButton
                              onClick={() => handleEditStart(gk.name)}
                              aria-label="Im Code verwendeten Namen bearbeiten"
                            >
                              <Pencil size={14} />
                            </WorkshopIconButton>
                          </Tooltip>
                          ...
                          <Tooltip content="Verbindung löschen" asChild>
                            <WorkshopIconButton
                              danger
        ...
        {!loading && hooks.length > 0 && (
          <section className="mt-8">
            <div className="mb-3">
              ...
              <p className="mt-1 text-[13px] leading-[18px] font-normal tracking-[-0.25px] text-kumo-subtle">
                Callbacks, über die verbundene Ressourcen dieses Gadget bei Ereignissen aufwecken können.
              </p>
            </div>
                  ...
                  >
                    {isDeleting ? (
                      <div className="flex flex-wrap items-center gap-3">
                        <div className="min-w-0 flex-1">
                          <p className="truncate text-[13px] leading-[18px] font-medium tracking-[-0.25px] text-kumo-danger">
                            Hook „{hook.description.title}" löschen?
                          </p>
                          <p className="truncate text-[12px] leading-4 font-normal tracking-[-0.2px] text-kumo-subtle">
                            Dadurch wird der Hook dauerhaft entfernt. Künftige Ereignisse werden nicht mehr zugestellt.
                          </p>
                        </div>
                        <WorkshopButton
                          tone="danger"
          ...
          try {
            ...
            toasts.add({
              title: chatId === undefined
                ...
                : 'Verbindung erstellt – übernimm die Änderungen des Chats, um sie zu behalten',
              variant: 'success',
            })
            await loadGatekeepers()
            onConnectionsChange?.()
...
function BlueprintAnnotationModal({
      ...
      try {
        ...
        if (!cancelled) {
          if (loaded) {
            ...
            setLoadError('Verbindung nicht gefunden.')
          }
        }
      } catch (err: any) {
        if (!cancelled) {
          reportIssue('connections.binding-load', err)
          setLoadError(err?.message || 'Verbindung konnte nicht geladen werden.')
        }
      }
    })()
    return () => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setSaveError(err?.message || 'Speichern nicht möglich.')
    } finally {
      setSaving(false)
    }
  }
        ...
        <Dialog className="responsive-dialog !z-[1000] !w-[min(480px,calc(100vw-32px))] overflow-hidden bg-kumo-base p-0" size="lg">
          <div className="flex items-start justify-between gap-4 border-b border-kumo-line px-4 py-4 sm:px-5">
            <div className="min-w-0">
              ...
              <Dialog.Description className="mt-1 text-[12px] leading-4 font-normal tracking-[-0.2px] text-kumo-subtle">
                Wie diese Verbindung in Blueprints erscheint.
              </Dialog.Description>
            </div>
            <Dialog.Close
              render={(props) => (
          ...
          <div className="space-y-4 px-4 py-4 sm:px-5">
            {loadError ? (
              ...
              <div className="py-2 text-center text-[13px] text-kumo-subtle">Wird geladen …</div>
            ) : (
              <>
                <BlueprintBindingCard
                  data={data}
