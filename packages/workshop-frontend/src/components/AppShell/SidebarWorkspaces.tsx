...
export function SidebarWorkspacesProvider({ children }: { children: ReactNode }) {
  ...
  const onTogglePin = useCallback(async (g: GadgetMetadataWithTimestamps) => {
    ...
    try {
      ...
      toasts.add({ title: 'Favorit konnte nicht aktualisiert werden', variant: 'error' })
    } finally {
  ...
  const onRename = useCallback(async (g: GadgetMetadataWithTimestamps, newTitle: string) => {
    ...
    try {
      ...
      toasts.add({ title: 'Arbeitsbereich konnte nicht umbenannt werden', variant: 'error' })
    } finally {
      overseer[Symbol.dispose]()
    }
  }, [authenticatedApi, toasts])
  ...
  const onShare = useCallback(async (g: GadgetMetadataWithTimestamps) => {
    ...
    try {
      ...
      toasts.add({ title: 'Freigabeeinstellungen konnten nicht geöffnet werden', variant: 'error' })
    }
  }, [authenticatedApi, toasts])
  ...
  const handleDeleteConfirm = useCallback(async () => {
    ...
    try {
      ...
      toasts.add({
        title: deleteTarget.owner ? 'Arbeitsbereich entfernt' : 'Arbeitsbereich gelöscht',
        variant: 'success',
      })
    } catch (err) {
  ...
  return (
    <WorkspacesContext.Provider value={value}>
      ...
      <DeleteConfirmationDialog
        ...
        title={deleteTarget?.owner ? 'Arbeitsbereich entfernen' : 'Arbeitsbereich löschen'}
        description={
          deleteTarget?.owner
            ? `„${deleteTarget?.title || 'Unbenannter Arbeitsbereich'}" aus deiner Liste entfernen? Du kannst ihn weiterhin über seinen Link öffnen.`
            : `„${deleteTarget?.title || 'Unbenannter Arbeitsbereich'}" löschen? Das kann nicht rückgängig gemacht werden.`
        }
        confirmLabel={deleteTarget?.owner ? 'Entfernen' : 'Löschen'}
        confirmingLabel={deleteTarget?.owner ? 'Wird entfernt…' : 'Wird gelöscht…'}
    ...
    <div className="flex flex-col pb-3">
      ...
      >
        {favorites.length === 0 ? (
          <p className="px-2.5 py-1.5 text-[12px] leading-4 tracking-[-0.2px] text-kumo-inactive">
            Favorisiere einen Arbeitsbereich, damit er hier bleibt.
          </p>
        ) : (
      ...
      >
        {gadgetsLoading ? (
          ...
          <p className="px-2.5 py-1.5 text-[12px] leading-4 tracking-[-0.2px] text-kumo-inactive">
            {search ? 'Keine Treffer.' : 'Noch keine Arbeitsbereiche.'}
          </p>
        ) : (
          <>
            <div className="flex flex-col">
            ...
            >
              {recentHidden > 0 ? `Alle anzeigen (${recent.length})` : 'Alle anzeigen'}
