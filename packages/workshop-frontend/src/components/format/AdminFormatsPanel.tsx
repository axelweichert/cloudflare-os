...
export default function AdminFormatsPanel({
  ...
  const mutate = async (op: () => Promise<void>) => {
    ...
    try {
      ...
      toasts.add({ title: 'Standardformate konnten nicht aktualisiert werden', variant: 'error' })
    } finally {
      setBusy(false)
    }
  }
  ...
  return (
    <div className="rounded-xl border border-kumo-line bg-kumo-elevated p-6">
      <h2 className="mb-1 text-lg font-semibold text-kumo-strong">Standardformate</h2>
      <p className="mb-5 text-sm text-kumo-subtle">
        Ein hochgestuftes Blueprint wird überall dort, wo Nutzer etwas beginnen, unter seinem Namen
        angeboten („Neues Doc“, „Neue Slides“), und der Agent wird angewiesen, es zu bevorzugen,
        statt dasselbe von Grund auf neu zu bauen.
      </p>
            ...
            >
              ...
              <span className="min-w-0">
                ...
                <span className="block truncate text-[11px] text-kumo-inactive">
                  {candidate.declared
                    ? `Erzeugt ${candidate.declared.plural}`
                    : 'Kein deklariertes Format. Du gibst ihm einen Namen.'}
                </span>
              </span>
            </DropdownMenu.Item>
          ))}
    ...
    <div className="mb-5 rounded-lg border border-dashed border-kumo-line bg-kumo-tint/40 p-4">
      ...
      {formats.length === 0 ? (
        <p className="text-[13px] italic text-kumo-inactive">
          Noch nichts. Nutzer sehen nur „Neuer Arbeitsbereich“.
        </p>
      ) : (
        <div className="flex flex-wrap items-center gap-2">
          {formats.map((format) => (
            ...
            >
              ...
              Neu {format.output!.noun}
            </span>
          ))}
        </div>
      )}
      <p className="mt-2.5 text-[12px] leading-4 text-kumo-subtle">
        Im +-Menü des Composers, in der Befehlspalette und auf einer leeren Ergebnisse-Seite, in
        dieser Reihenfolge.
      </p>
    </div>
  )
...
function EmptyState() {
  return (
    <div className="mb-5 rounded-lg border border-kumo-line bg-kumo-base px-4 py-5 text-center">
      <p className="text-sm font-medium text-kumo-default">Noch keine Standardformate</p>
      <p className="mx-auto mt-1 max-w-md text-[13px] leading-[18px] text-kumo-subtle">
        Stufe ein Blueprint hoch, um es überall dort, wo Nutzer etwas beginnen, unter seinem Namen
        anzubieten, und damit der Agent es bevorzugt, statt dasselbe von Grund auf neu zu bauen.
      </p>
    </div>
  )
}
  ...
  return (
    <div className="group rounded-lg border border-kumo-line bg-kumo-base">
      <div className="flex items-center gap-3 p-3">
          ...
          <span className="min-w-0 flex-1">
            <span className="flex items-center gap-2">
              <span className="truncate text-sm font-medium text-kumo-default">
                {format.output ? `Neu ${format.output.noun}` : format.blueprintTitle || format.blueprintId}
              </span>
              {format.bundled && <Badge>Gebündelt</Badge>}
              {!format.enabled && !format.missing && <Badge>Aus</Badge>}
              {needsNaming && <Badge tone="warn">Braucht einen Namen</Badge>}
            </span>
        ...
        >
          <IconButton label="Nach oben verschieben" disabled={busy || isFirst} onClick={() => onMove(-1)}>
            <ArrowUp size={13} />
          </IconButton>
          <IconButton label="Nach unten verschieben" disabled={busy || isLast} onClick={() => onMove(1)}>
            <ArrowDown size={13} />
          </IconButton>
        </div>
        ...
        <div className="flex flex-col gap-4 border-t border-kumo-line px-3 py-4">
          {format.missing ? (
            <p className="text-[13px] text-kumo-subtle">
              Das Blueprint hinter diesem Format wurde gelöscht, daher wird es niemandem angeboten.
            ...
            <>
              ...
              >
                <OverrideField
                  label="Hinweis"
                  placeholder="z. B. für kundenorientierte Decks bevorzugen"
              ...
              <div className="flex items-end justify-between gap-4 border-t border-kumo-line pt-3">
                <p className="text-[12px] leading-4 text-kumo-subtle">
                  {(format.enabled
                    ? 'Wenn du dies ausschaltest, wird es aus den Menüs oben und aus dem Katalog des Agents entfernt. Bereits daraus erstellte Ergebnisse funktionieren weiterhin. '
                    : 'Aktuell in den Menüs oben und im Katalog des Agents ausgeblendet. ') +
                    (format.bundled
                      ? 'Es wird mit dem Deployment ausgeliefert und bleibt daher in jedem Fall in dieser Liste.'
                      : '')}
                </p>
                {!format.bundled && (
  ...
  return (
    <label className="flex flex-col gap-1">
      <span className="text-[11px] font-medium uppercase tracking-[0.06em] text-kumo-inactive">
        {label}
        {overridden && (
          <span className="ml-1 normal-case tracking-normal text-kumo-subtle">(überschrieben)</span>
        )}
      </span>
      <Input
        value={draft}
