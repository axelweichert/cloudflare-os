        ...
        <DropdownMenu>
          ...
          <DropdownMenu.Content className={MENU_CONTENT}>
            <DropdownMenu.Item onClick={() => onTogglePin(item)} className={MENU_ITEM}>
              ...
              {item.pinned ? 'Aus Favoriten entfernen' : 'Favorisieren'}
            </DropdownMenu.Item>
            {item.inLibrary && (
        ...
        <div className="mb-4 flex flex-col gap-2 px-3 sm:flex-row sm:items-center">
          ...
          <div className="grid w-full grid-cols-2 gap-2 sm:w-auto sm:shrink-0">
            ...
            <button
              ...
              disabled={uploading}
              title="Ein .gadget-Archiv hochladen"
      ...
      <div className="chat-panel flex min-h-0 flex-1 flex-col gap-0.5 overflow-y-auto pt-1">
        {loading ? (
          ...
          <div className="py-12 text-center text-sm">
            <p className="text-kumo-danger">Beim Laden deiner Blueprints ist etwas schiefgelaufen.</p>
            <button type="button" onClick={load} className="mt-1 text-kumo-brand underline">Erneut versuchen</button>
          </div>
        ) : filtered.length === 0 ? (
          search ? (
            <div className="py-12 text-center text-sm text-kumo-inactive">Keine Blueprints gefunden</div>
          ) : (
            <div className="flex flex-col items-center gap-3 px-3 py-16 text-center">
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                >
                  <UploadSimple size={14} weight="bold" />
                  {uploading ? 'Wird hochgeladen …' : '.gadget hochladen'}
                </button>
              </div>
            </div>
          )
