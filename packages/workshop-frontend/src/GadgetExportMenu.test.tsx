...
describe('GadgetExportMenu', () => {
  ...
  it('disables the export button and updates its tooltip while exporting', async () => {
    ...
    const trigger = container.querySelector<HTMLButtonElement>('[aria-label="Gadget exportieren"]')
    expect(trigger?.disabled).toBe(true)
    expect(trigger?.closest('[data-tooltip]')?.getAttribute('data-tooltip')).toBe('Wird als CSV exportiert')
    ...
    expect(trigger?.closest('[data-tooltip]')?.getAttribute('data-tooltip')).toBe('Gadget exportieren')
  })
  ...
  it('shows an empty state without hiding or disabling the export button', async () => {
    ...
    expect(container.textContent).toContain('Dieses Gadget unterstützt keine Exporte.')
    expect(container.querySelector('[aria-label="Gadget exportieren"]')).not.toBeNull()
  })
  ...
  it('does not render the control without a selected Gadget', async () => {
    ...
    expect(container.querySelector('[aria-label="Gadget exportieren"]')).toBeNull()
  })
  ...
  it('loads fresh formats on every open and ignores a response after close', async () => {
    ...
    expect(container.querySelector('[role="status"][aria-label="Exportformate werden geladen"]')).not.toBeNull()
    await act(async () => { button('close export menu')?.click() })
    expect(container.querySelector('[role="status"][aria-label="Exportformate werden geladen"]')).toBeNull()
  ...
  it('shows an inline error and retries format discovery', async () => {
    ...
    try {
      ...
      expect(container.textContent).toContain('Exportformate konnten nicht geladen werden.')
