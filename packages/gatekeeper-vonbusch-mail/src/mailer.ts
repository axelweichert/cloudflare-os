// vonBuschOS — Mail-Gatekeeper (K5-Port, VON-1818): Versand-Adapter
//
// Portiert 1:1 aus `vonbusch/mail-gatekeeper/mailer.ts` (VON-1802). Baut eine simple
// RFC-822/2822 Nachricht (nur text/plain, UTF-8) und versendet sie über die CF `send_email`-
// Bindung. Der MIME-Bau ist rein & getestet; der reale Versand ist hinter dem `Mailer`-Interface
// gekapselt und in Tests injizierbar. Aufgerufen wird der Versand ausschließlich aus
// `MailGatekeeper.applyAction()` — also erst NACH menschlicher Freigabe über die OS-ApprovalQueue.

import type { EmailProposal } from "./mail-actions";
 ...
 * `EmailMessage` und die Bindung stehen nur zur Laufzeit auf workerd zur Verfügung, daher wird
