...
function seeded(): MemoryCrmStore {
  ...
  s.seed("contact", [
    { id: "c1", first_name: "Erika", last_name: "Mustermann", email: "erika@acme.de" },
    { id: "c2", first_name: "Max", last_name: "Beispiel", email: "max@globex.de" },
  ]);
  s.seed("deal", [
...
test("D1 read: parametrisiert mit LIMIT/OFFSET und Suche", async () => {
  ...
  const c = calls[0];
  assert.match(c.sql, /SELECT \* FROM contacts WHERE \(first_name LIKE \? OR last_name LIKE \? OR email LIKE \?\) LIMIT \? OFFSET \?/);
